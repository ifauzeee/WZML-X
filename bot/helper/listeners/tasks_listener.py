...
class MirrorLeechListener:
    ...
    async def onDownloadComplete(self):
        ...
        if self.isLeech:
            ...
            # PERUBAHAN: Mengirim 'self' (listener) ke TelegramStatus
            ...
            # PERUBAHAN: Mengirim 'self' (listener) ke GdriveStatus
            ...
            # PERUBAHAN: Mengirim 'self' (listener) ke DDLStatus
            ...
            # PERUBAHAN: Mengirim 'self' (listener) ke RcloneStatus
            async with download_dict_lock:
...
┖ <b>Elapsed:</b> {get_readable_time(time() - self.message.date.timestamp())}"""
        ...
        async with queue_dict_lock:
            if self.uid in queued_dl:
                queued_dl[self.uid].set(); del queued_dl[self.uid]
            if self.uid in queued_up:
                queued_up[self.uid].set(); del queued_up[self.uid]
            if self.uid in non_queued_dl:
                non_queued_dl.remove(self.uid)
            if self.uid in non_queued_up:
                non_queued_up.remove(self.uid)
